...
fn SpriteApp(hooks: &mut Hooks) -> Element {
    ...
    let global_pixie_scale: i32 = 4; // changed to 4
    ...
    let pixiestrings = ambient_api::element::use_query(hooks, (pix_string(), pix_place(), pix_z()));
    ...
    Group::el([
        ...
        Group::el(pixiestrings.iter().map(move |(pixie, (string, place, z))| {
            Text::el(string).with(
                translation(),
                (*place * global_pixie_scale + global_pixie_offset)
                    .as_vec2()
